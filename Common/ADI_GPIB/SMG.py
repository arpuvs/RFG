...
# ROHDE & SCHWARZ SMG Generators
from ADI_GPIB.GPIBObject import GPIBObjectBaseClass as GPIBObjectBaseClass
import time

class SMG(GPIBObjectBaseClass):
    def __init__(self, addr=-1, delay=0.025):
        GPIBObjectBaseClass.__init__(self, 'ROHDE&SCHWARZ,SMG', addr)
        self.__delay__ = delay
        
    def __SetFrequency__(self, Frequency):
        '''This function sets the SMG frequency'''
        self.instr.write('IECOUT27,RF ' + str(Frequency))
        time.sleep(self.__delay__)
    
    def __SetLevel__(self, Level):
        '''This function sets the SMG frequency'''
        self.instr.write('IECOUT27,LEVEL ' + str(Level))
        time.sleep(self.__delay__)

    def __GetLevel__(self):
        '''This function gets the SMG level'''
        return float( self.instr.ask('IECOUT27,LEVEL?')[11:] )
    ...
    Frequency = property(None, __SetFrequency__, None, "Sets the SMG Frequency")
    Level     = property(__GetLevel__, __SetLevel__,     None, "Sets the SMG Level")
    Enabled   = property(None, __SetEnable__,    None, "Sets the SMG RF On/Off")
...
class GPIB_DB_SMG( GPIB_Dashboard.Utils.InstrumentBaseClass ):
    def __init__(self):
        self.SetFunction( 'Generator' )
        self.SetName( 'SMG' )
        self.SetAddress( 0 )
        self.this_driver = SMG

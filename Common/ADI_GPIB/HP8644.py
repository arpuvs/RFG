...
# HEWLETT-PACKARD HP8644 Generators
#
# Modifications:
#       6/2/2008 by Dave Shoudy to add phase control for the RF output
#       6/24/2010 by Kaushal Shrestha to add GetFrequency Control
from ADI_GPIB.GPIBObject import GPIBObjectBaseClass as GPIBObjectBaseClass
import time

class HP8644(GPIBObjectBaseClass):
    def __init__(self, addr=-1, delay=0.5):
        GPIBObjectBaseClass.__init__(self, 'HEWLETT_PACKARD,8644', addr)
        self.__delay__ = delay
        
    def __SetFrequency__(self, Frequency):
        '''This function sets the SMG frequency'''
        self.instr.write('FREQ:CW ' + str(Frequency / 1e6) + 'MHZ')
        time.sleep(self.__delay__)

    def __GetFrequency__(self):
        '''This function gets the SMG frequency'''
        return float( self.instr.ask('FREQ:CW?') )
    
    def __SetLevel__(self, Level):
        '''This function sets the SMG frequency'''
        self.instr.write('AMPL:LEV ' + str(Level))
        time.sleep(self.__delay__)
    
    def __GetLevel__(self):
        '''This function gets the SMG frequency'''
        return float( self.instr.ask('AMPL:LEV?') )
        
    def __SetEnable__(self, Enabled):
        '''This function enables the RF On/Off for the SMG'''
        if Enabled:
            self.instr.write('AMPL:STAT ON')
        else:
            self.instr.write('AMPL:STAT OFF')    
        time.sleep(self.__delay__)
    ...
    def __SetPhase__(self,degrees):
        '''Sets the output phase'''
        rad = degrees*3.14159265/180
        self.instr.write('PHAS %s' %str(rad))
    
    def __GetPhase__(self):
        '''This function returns the phase in degrees from the generator'''
        rad_phase = self.instr.ask('PHAS?')
        deg_phase = float(rad_phase)*180/3.14159265
        return deg_phase
        
    
    Frequency = property(__GetFrequency__,__SetFrequency__, None, "Sets the HP8644 Frequency")
    Level     = property(__GetLevel__, __SetLevel__,     None, "Sets the HP8644 Level")
    Enabled   = property(None,         __SetEnable__,    None, "Sets the HP8644 RF On/Off")
...
class GPIB_DB_HP8644( GPIB_Dashboard.Utils.InstrumentBaseClass ):
    def __init__(self):
        self.SetFunction( 'Generator' )
        self.SetName( 'HP8644' )
        self.SetAddress( 0 )
        self.this_driver = HP8644

...
# update:  2/2/2016 Lloyd Weida  add functions:  set_Phase_Interference  SET_OUTPUT_INTERFACE  
#                                correct Rj freq bandwith setting of function: Random_Jitter_Freq
...
class TekBSA175C(GPIBObjectBaseClass):
    def __init__(self, addr=-3):
...
##        return float(self.instr.ask("GSM:SJitter:AMPUi?"))    
    ...
    def get_gen_rate(self):
        return float(self.instr.ask("GEN:ICL?"))    
    ...
    def set_Phase_Interference(self,Phase): 
        self.instr.write("GSM:SInterference:MODE " +str(Phase)) #set phase to INPhase, OUTPhase, or EXTErnal            
    ...
    def Random_Jitter_Amp(self,randomAmp):
        self.instr.write("GSM:RJitter:AMPU " +str(randomAmp)) #sets amplitude 
           
    def Random_Jitter_Freq(self,randomFreq,randomAmp):
        self.instr.write("GSM:RJ:TYPE " +str(randomFreq)) #sets adjust frequency bandwidth either: 1GHZ or 100MHZ
    ...
    def BUJ_Jitter_Amp(self,bujAmp):
    ...
    def get_ck_Divider(self):
        return float(self.instr.ask("GEN:CLKDIV?"))               
    ...
    def SET_OUTPUT_INTERFACE(self,mode):
        self.instr.write("STRCmb:OUTInf " +str(mode)) #SET BSAITS125 TO SINGLE or to DIFFERENTIAL          

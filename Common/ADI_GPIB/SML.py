...
# ROHDE & SCHWARZ SML Generators
from ADI_GPIB.GPIBObject import GPIBObjectBaseClass as GPIBObjectBaseClass
import time

class SML(GPIBObjectBaseClass):
    def __init__(self, addr=-1, delay=0.01):
        GPIBObjectBaseClass.__init__(self, 'ROHDE&SCHWARZ,SML', addr)
        self.__delay__ = delay
        
    def __SetFrequency__(self, Frequency):
        '''This function sets the SML frequency'''
        self.instr.write( 'FREQ %sMHz' % (str(Frequency / 1e6)) )
        time.sleep(self.__delay__)
    
    def __SetLevel__(self, Level):
        '''This function sets the SML level'''
        self.instr.write( 'POW %sdBm' % (str(Level)) )
        time.sleep(self.__delay__)
    
    def __SetEnable__(self, Enabled):
        '''This function enables the RF On/Off for the SML'''
        if Enabled:
    ...
    Frequency = property(None, __SetFrequency__, None, "Sets the SML Frequency")
    Level     = property(None, __SetLevel__,     None, "Sets the SML Level")
    Enabled   = property(None, __SetEnable__,    None, "Sets the SML RF On/Off")
...
class GPIB_DB_SML( GPIB_Dashboard.Utils.InstrumentBaseClass ):
    def __init__(self):
        self.SetFunction( 'Generator' )
        self.SetName( 'SML' )
        self.SetAddress( 0 )
        self.this_driver = SML

...
def IMD3main(path, freqlist, vcomlist, templist, dut):
    ...
    def IMD3():
        ...
        for freq in freqlist:
            ...
            # Measure lower IMD3
            instDict['source1'].__SetState__(1)
            instDict['SA'].__Setfc__(freq - 3*delta)
            if pkSearch == True:
                instDict['SA'].__PeakSearch__(1)
            else:
                instDict['SA'].__SetMarkerFreq__(1, freq - 3*delta)
            # raw_input('Waiting...')
            instDict['SA'].__ClearAverage__()
            instDict['SA'].__CheckStatus__(300)
            ...
            supplyI.append(float(instDict['Supply'].__GetI__(1)))
    ...
    instDict['Supply'].__SetV__(5, 1)
    instDict['Supply'].__SetI__(0.25, 1)
    ...
    # for balun in balunList:
        ...
        for vcom in vcomlist:
            instDict['Supply'].__SetV__(vcom, 3)  # Sets DUT to common mode voltage
